/**
 * In-memory data store — seeded with the same mock data from the legacy
 * GarageOS prototype. All mutations go through this module so every
 * endpoint reads from a single shared source of truth.
 */
...
// ─── Labor catalog (used by jobs) ────────────────────────────────────────────

export const laborCatalog: LaborCharge[] = [
...
export const jobCards: JobCard[] = [
    { id: "JC-1042", registration: "KDK 420X", customer: "James Mutiso",  phone: "0722 100 220", mechanic: "Peter Kamau",   stage: "diagnostics", startedAt: Date.now() -  12 * 60_000, faults: "Knocking sound on front left, pulls to the right when braking.", lines: [] },
...
// ─── Customers (staff view — vehicle roster) ─────────────────────────────────

export const customerRecords: CustomerVehicleRecord[] = [
...
// ─── Customer vehicles (customer app view — full detail) ─────────────────────
...
export const customerNotifications: CustomerNotification[] = [
    ...
    { id: 2, type: "reminder", read: false, time: "Today, 07:00", title: "Service due in 19 days",         body: "Your Fielder (KCB 118Q) is due for service by 02 Oct 2026 or 117,000 km."                         },
...
export interface BusinessSettings {
    name: string;
    kra: string;
    vatRate: number; // percent, e.g. 16
}

export const settings: BusinessSettings = {
    name: "Kamau & Sons Auto Garage",
    kra: "P051234567X",
    vatRate: 16,
};

// ─── POS cart (server-side session — single cashier MVP) ─────────────────────
...
export let posCart: CartLine[] = [];

export const clearCart = () => { posCart = []; };
